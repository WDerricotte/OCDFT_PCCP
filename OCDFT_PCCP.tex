...
In this work we have extended OCDFT in order to calculate multiple core excited states. Our OCDFT implementation ensures mutual orthogonality between all excited states by imposing a set of conditions that produce a series of orthogonal Kohn-Sham solutions. We target core excitations specifically by selecting the lowest lying solutions to the OCDFT eigenvalue equation (NOTE: not correct, need to clarify). We show that standard functionals can be used with OCDFT to accurately compute core-excited states. A formal comparison with CIS reveals that OCDFT provides a better description of core-valence excitations due to the local nature of the integrals present in the excitation energy expression (NOTE: misleading, sounds like OCDFT is more accurate than CIS).  Benchmarking OCDFT computations on a range of first and second row core excitation energies using conventional pure and hybrid functionals showed no significant dependence on the amount of HF exchange (NOTE: as it is written the sentence is not clear). 

NOTE: summarize the next few sentences and try to avoid the use of ``increase''.  We show that it is helpful to examine first row and second row core excitations independently, due to the drastic decrease in the quality of the TDDFT core excitation energies. The MAE of the TDDFT core excitation energies increases by 20.0 eV when considering excitations from second row elements, while the MAE of the OCDFT excitation energies only increases by 1.0 eV. When considering first-row elements, the MAE of OCDFT is 11.1 eV lower than that of TDDFT. For the second-row the difference is even greater, with OCDFT producing an MAE 30.0 eV lower than that of TDDFT. 
\begin{comment}
...
Our OCDFT NEXAS spectra of thymine and adenine are in excellent agreement with the experimentally measured spectra. Our calculations show distinct $\pi^*$ transitions in the lower energy regime and significant mixing between the $\pi^*$ and diffuse orbitals in the higher energy regime of the spectra, which agrees well with previous NEXAS studies of these molecules.\cite{plekan_theoretical_2008,wenzel_calculating_2014}
OCDFT assignments for the transitions of highest oscillator strength are in good agreement with the experimental assignments for these transitions (NOTE: too many repetitions, rephrase).
