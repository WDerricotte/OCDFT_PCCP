...
\noindent \normalsize{Here we consider a variational, time-independent density functional theory (DFT) approach to the calculation of core-excited states. Orthogonality constrained density functional theory (OCDFT) (F. A. Evangelista, P. Shushkov and J. C. Tully, \textit{J. Phys. Chem. A}, 2013, {\bf{117}}, 7378-7392) is an extension of the variational DFT approach that imposes orthogonality conditions onto the wavefunctions within the framework of a generalized Kohn-Sham system. This method is favorable because it is relatively cheap, allows for spin adapted solutions, and does not suffer from the variational collapse. We present extensions to the theory by implementing two additional features. First we modify our selection of solutions to the original OCDFT eigenvalue equation in order to allow for the calculation of core excitation energies. Second we add the ability to calculate multiple excited states which involves imposing additional orthogonality conditions for each excited state. A benchmarking test set of 40 core excitations from 13 different molecules was used to assess the accuracy of the excitation energies obtained. Comparisons are made to time-independent DFT methods in order to showcase the superior accuracy of OCDFT when utilizing traditional density functionals (B3LYP, BLYP, and PBE0). We show that OCDFT can simulate the near-edge x-ray absorption spectra (NEXAS) of larger molecules by simulating the gas-phase near-edge spectra of adenine and thymine.}
\end{@twocolumnfalse}
\vspace{0.5cm} ]
%\doublespacing
...
Eq. \eqref{osc} yields the absolute oscillator strength ($f_{\rm abs}$) for a given transition. The intensity of the spectra is then scaled relative to the most intense peak, we will refer to these scaled values as the \textit{relative oscillator strength} ($f_{\rm rel}$).
Natural spectroscopic broadening effects are simulated by convoluting the OCDFT peaks with a Gaussian function whose exponent was calibrated to best fit the experimental spectra.
...
Individual excitation energies computed with OCDFT and TDDFT are compared to values from gas-phase NEXAS experiments \cite{puttner_vibrationally_1999,remmers_high-resolution_1992,chen_k-shell_1989,tronc_nitrogen_1980,tronc_carbon_1979,francis_studies_1994,adachi_vibronic_1999,hitchcock_k-shell_1979,domke_carbon_1990,nayandin_angle-resolved_2001,bodeur_single-and_1990,gedat_s_1998,hudson_high-resolution_1994,cavell_chemical_1999,bodeur_photoabsorption_1985} in Tables \ref{table:FirstRow} and \ref{table:SecondRow}. Table \ref{table:FirstRow} displays excitations from first-row elements, while Table \ref{table:SecondRow} shows excitations from second-row elements. For highly symmetric molecules (N$_2$, C$_2$H$_2$, and Cl$_2$), we examined solutions with and without a broken symmetry wavefunction. When considering a wavefunction without broken symmetry, the excitation energies for the molecules with symmetry equivalent atoms produce an MAE of 13.7 eV when compared to experiment. When considering the broken symmetry solution the performance improves, decreasing the MAE to 0.9 eV. Thus all calculations for highly symmetric molecules in Tables \ref{table:FirstRow} and \ref{table:SecondRow} utilize a wavefunction with broken symmetry.
...
\subsubsection{Adenine Nitrogen K-Edge}
   \ Figure \ref{figure:Adenine}a displays the nitrogen k-edge of adenine computed with OCDFT. The most dominant feature of the spectra, peak A, is experimentally classified at 399.5 eV, OCDFT predicts that this feature is a result of core-valence excitations originating from three of the four nitrogens on the purine ring system (N$_4$, N$_3$, N$_5$). The largest contributor being a transition from N$_5$ $\rightarrow$ $\phi_{\pi_2^*}$ predicted at 399.4 eV, which agrees well with the experimental classification for the peak. An apparent shoulder feature with fairly weak oscillator strength is present in the experimental spectra in the region from 399.8 -- 400.4 eV. OCDFT represent this feature well, with two weak $\pi^*$ resonances resulting from the two nitrogens on the six-membered ring. A few rising shoulder features are shown in the experimental spectra in the region from 401.0 eV -- 401.3 eV, OCDFT predicts a N$_3$ $\rightarrow$ $\phi_{\pi_2^*}$ transition in this region with a relative oscillator strength of 0.109. Peak B is a mixture of transitions to $\pi^*$ orbitals as well as diffuse orbitals, with the $\pi^*$ resonances being the prominent contributors. 
...
The OCDFT carbon and nitrogen K-edge spectra were convoluted with a Gaussian function with full width at half maximum (FWHM) equal to 0.3 eV.  The experimental spectra is reproduced with permission from Ref. \citenum{plekan_theoretical_2008}.}
...
Oxygen  & O$_2$ $\rightarrow$ $\pi^*_1$ (531.05) &  O$_2$ $\rightarrow$ $\pi^*_1$ (531.36)  \\
Nitrogen  & N$_4$ $\rightarrow$ $\pi^*_1$ (401.18) & N$_4$ $\rightarrow$ $\pi^*_1$ (401.47) \\
Carbon  & C$_5$ $\rightarrow$ $\pi^*_2$ (289.14) & C$_5$ $\rightarrow$ $\pi^*_2$ (289.68) 
\\[2pt]
\multicolumn{3}{c}{\bf Adenine} \\
Nitrogen &  N$_5$ $\rightarrow$ $\pi^*_2$ (399.42) & N$_5$ $\rightarrow$ $\pi^*_2$ (399.64) \\
Carbon  & C$_6$ $\rightarrow$ $\pi^*_1$ (287.27) & C$_6$ $\rightarrow$ $\pi^*_1$ (287.37)\\[2pt]
\hline
\hline
\end{tabular}
...
In this work we have extended OCDFT in order to calculate multiple core excited states. Our OCDFT implementation ensures mutual orthogonality between all excited states by imposing a set of conditions that produce a series of orthogonal Kohn-Sham solutions. {\bf{We target core excitations specifically by selecting the smallest eigenvalues of the OCDFT eigenvalue equation that defines the core orbital in the excitation.}} We show that standard functionals can be used with OCDFT to accurately compute core-excited states. A formal comparison with CIS reveals that OCDFT provides a better description of core-valence excitations than TDDFT due to the local nature of the integrals present in the excitation energy expression.  Benchmark OCDFT computations on a range of first and second row core excitation energies using conventional pure and hybrid functionals showed {\bf{that OCDFT has}} no significant dependence on the amount of HF exchange.

 We show that it is helpful to examine first row and second row core excitations independently, {\bf{because of the drastic 20 eV change in the MAE of the TDDFT core excitation energies. The performance of OCDFT is more consistent, the MAE between first and second row core excitation energies only differs by 1.0 eV. The MAE of OCDFT is also favorable over TDDFT in each separate case, producing an MAE that is 11.1 eV lower for first row and 30.0 eV lower for second row core excitation energies.}}
\begin{comment}
OCDFT also performed well across both the first and second row of the periodic table, only losing approximately 1.0 eV of accuracy when moving to the second row of the periodic table. Also outperforming TDDFT by ~11.1 eV when calculating first row core excitations, and by ~30.0 eV when calculating second row core excitations.
\end{comment}

Our OCDFT NEXAS spectra of thymine and adenine are in excellent agreement with the experimental spectra. Our calculations show distinct $\pi^*$ transitions in the lower energy regime and significant mixing between the $\pi^*$ and diffuse orbitals in the higher energy regime of the spectra, {\bf{which are characteristic features of the NEXAS spectra of these molecules.\cite{plekan_theoretical_2008,wenzel_calculating_2014} The OCDFT assignments of the spectral features are shown to be commensurate with the assignments made using ADC(2),\cite{plekan_theoretical_2008} showing that OCDFT is an accurate and useful tool for interpreting experimental NEXAS spectra.}}

OCDFT may be a favorable method for core electron calculations in other phases of matter beyond the gas-phase. Current DFT methods that employ a plane-wave basis are fundamentally flawed for XAS computations because they represent the core electrons and nuclei with a pseudopotential. Resulting in a necessary shift to the excitation energies that is either arbitrary\cite{hetenyi_calculation_2004,taillefumier_x-ray_2002} or computed \cite{cavalleri_x-ray_2004} in order to make a comparison with experiment. We have shown that OCDFT is efficient and accurate at simulating NEXAS spectra in the gas-phase. An extension to liquid and solid phase should be possible without the need of a pseudopotential. 

The relativistic correction employed in this study is an \textit{ad-hoc} method, a more robust treatment of relativistic effects would likely improve the accuracy of the computed core excited states in OCDFT, and allow it to be more easily applicable to larger atoms. We are currently working on the implementation of a scalar relativistic Hamiltonian, and will investigate these excitations further using this more sophisticated relativistic treatment.
