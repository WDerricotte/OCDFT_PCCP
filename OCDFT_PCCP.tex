...
%\fancyfoot[CO]{\vspace{-7.2pt}\hspace{8.2cm}\includegraphics{headers/RF}}
...
\footnotetext{\dag~Electronic Supplementary Information (ESI) available: Cartesian geometries for adenine and thymine optimized at the B3LYP/def2-TZVP level of theory, tables reporting all computed excitation energies of adenine and thymine. See DOI: 10.1039/b000000x/}
...
%    \begin{tabular*}{8cm}{@{\extracolsep{\fill} }llcrrHH}
    \hline
    \hline
     Molecule & Excitation                     & Exp. (eV) & \multicolumn{2}{c}{Error (eV)} \\ ~&~ &~   & TDDFT  & OCDFT & $f_{abs} \times 10^{-2}$\\
     \hline
...
\small
    ...
    \centering
    \begin{tabular*}{11.5cm}{@{\extracolsep{\fill} }llrrrrHH}
...
% \parbox[c]{1em}{
 ...
 &   64.0$\%$ $\pi_2^*$  & 532.1 & 0.968 & B & 532.3 \vspace{0.05in}\\
    O$_1$
 &   71.2$\%$ $\pi_1^*$  & 533.4 & 0.146 & \multirow{2}{*}{B$^{\prime}$} & \multirow{2}{*}{533.8}  \\ % $\approx$
    O$_2$
...
\vspace{0.05in}\\
    N$_5$
 &   92.4$\%$ $\pi_1^*$  & 399.7 & 0.002 & \multirow{2}{*}{A$^{\prime}$} & \multirow{2}{*}{400.4}  \\ %$\approx$ 
    N$_4$
 &   98.9$\%$ $\pi_2^*$
 & 400.4 & 0.022 
 \vspace{0.05in}\\
...
OCDFT excitation energies are slightly more accurate for first-row elements (MAE = 0.4 eV) than second-row elements (MAE = 1.6 eV). \hl{There are a few potential sources of the remaining error in the OCDFT excitation energies. First is the fact that the excited state potential is constructed using the exchange-correlation functional from the ground state calculation. This means, we are not constructing the true excited state, but rather an approximate excited state. Secondly, the standard exchange-correlation functionals used in this study utilize approximate exchange and the exact functional is unknown. This error plagues all density functional techniques and is always taken into consideration. Thirdly our unrefined treatment of relativistic effects can only include the total relaxation of the core-hole and neglects all other relaxation effects. Lastly we note that we do not treat certain spectroscopic considerations such as vibrational effects that are present in the experiment.}
...
\begin{mdframed}[hidealllines=true,backgroundcolor=blue!20]
Since we have definitively proven that OCDFT is a precise method for calculating core-excited states in first- and second-row elements, it is logical to begin work toward extending the theory to treat more complex systems. Two classes of systems that are worth further exploration are transition metal complexes and open-shell molecules. Core excitations in transition metal complexes are more challenging and will require further extension of the theory. One of the biggest improvements that will be neccesary is a proper treatment of relativistic effects, the method used in this study was a very simple additive correction that can, at most, account for the relativistic contraction of the 1s orbital and omits the relaxation effects of all other orbitals. While this unrefined scheme was sufficient for first- and second-row elements, it will likely prove ineffective for the treatment of transition metals where relativistic effects are more important.
In the future we plan to combine OCDFT with an approximate relativistic Hamiltonian. 
A more robust treatment of relativistic effects would likely improve the accuracy of the computed OCDFT core-excited states, in particular for third- and higher-row elements. 2p excitations in transition metals are challenging due to the degeneracy of the 2p core hole. This means that multiple configurations may be necessary for an accurate description of the 2p excitation spectrum. While OCDFT is ideal for excitations with dominant contribution from a single electronic configuration, it can be readily extended to treat excited states with multiconfigurational character. In the original formulation of OCDFT, we highlight a spin adaptation procedure that incorporates a CI wavefunction. \cite{evangelista_orthogonality_2013} Considering multiple configurations in the CI wavefunction will readily extend OCDFT to handle the type of configuration mixing that arises in the 2p excitation spectrum. 
\end{mdframed}


\begin{mdframed}[hidealllines=true,backgroundcolor=blue!20]

There are two possibilities in order to extend OCDFT to treat open-shell molecules. One possibility is to implement a restricted open-shell Kohn--Sham (ROKS) formalism in OCDFT. However this approach has received criticism due to the essential unrestricted character of Kohn--Sham orbitals for open-shell systems. \cite{pople_spin-unrestricted_1995} Nonetheless, there have still been studies done computing excitation energies with ROKS with varying degrees of success. \cite{filatov_application_1999,grimm_restricted_2003} We can also utilize an unrestricted Kohn--Sham formalism, An accurate unrestricted  OCDFT formalism to treat  open-shell systems would require a spin-adapted mixed spin solution. Spin adaptation was explored in two ways in our original implementation of OCDFT:\cite{evangelista_orthogonality_2013} 1) Involved using a spin raising operator to express the triplet state in terms of the mixed spin solution and 2) Computed the singlet state using configuration interaction and introduces spin adaption by using a linear combination of the true mixed state wave function and a spin flipped wave function obtained by interchanging alpha and beta spins of the true wave function. These two methods of spin adaptation have been tested on a class of singlet valence-valence excitations and show comparable accuracy to TDDFT.\cite{evangelista_orthogonality_2013} These spin-adaptation procedures should aid in our development an unrestricted theory to treat open-shell systems.
\end{mdframed}
