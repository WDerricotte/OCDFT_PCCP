...
\noindent\large{\textbf{Wallace D. Derricotte and Francesco A. Evangelista$^{\ast \dag}$}}\vspace{0.5cm}
%Please note that \ast indicates the corresponding author(s) but no footnote text is required. 

\footnotetext{\dag~Electronic Supplementary Information (ESI) available: Cartesian geometries for adenine and thymine optimized at the B3LYP/def2-TZVP level of theory, full tables displaying every transition that contributes to our calculated NEXAS spectra of adenine and thymine. See DOI: 10.1039/b000000x/}
...
We show that OCDFT is a powerful tool for the assignment of X-ray absorption spectra of larger molecules by simulating the gas-phase near-edge spectra of adenine and thymine.}
%\end{@twocolumnfalse}
\vspace{0.5cm}
%\doublespacing
...
The optimized geometries and the NEXAS spectra of adenine and thymine were computed at the def2-TZVP/B3LYP level of theory (cartesian geometries are provided in the supplementary material).
...
\begin{center}
\begin{tabular*}{0.55\textwidth}{@{\extracolsep{\fill} }llcrrHH}
...
\small
    ...
    \centering
    \begin{tabular*}{0.525\textwidth}{@{\extracolsep{\fill} }llrrrHH}
...
% \parbox[c]{1em}{
 ...
 &   64.0$\%$ $\pi_2^*$  & 532.08 & 0.968 & B & 532.3 \vspace{0.05in}\\
    O$_1$
 &   71.2$\%$ $\pi_1^*$  & 533.38 & 0.146 & \multirow{2}{*}{B$^{\prime}$} & \multirow{2}{*}{ 533.8}  \\ % $\approx$
    O$_2$
...
\vspace{0.05in}\\
    N$_5$
 &   92.4$\%$ $\pi_1^*$  & 399.69 & 0.002 & \multirow{2}{*}{A$^{\prime}$} & \multirow{2}{*}{400.4}  \\ %$\approx$ 
    N$_4$
 &   98.9$\%$ $\pi_2^*$
 & 400.39 & 0.022 
 \vspace{0.05in}\\
...
In this work we have extended OCDFT in order to calculate multiple core-excited states.
...
In comparison, excitation energies computed with TDDFT are significantly less accurate and degrade as one goes from first-row (MAE = 11.6 eV) to second-row elements (MAE = 31.6 eV).
...
From the computational point of view, the scaling of OCDFT vs. the number of electrons ($N$) is identical to that of ground state DFT ($N^3$ and $N^4$ for pure and hybrid functionals) and it is lower than that of approximate coupled cluster (e.g. CC2) and Green's function methods[ADC(2)], which scale as $N^5$.
