...
\appendix
\section{Appendix}
Next, we perform a series of $n_{\rm u}$ OCDFT computations in which we solve the set of equations
\begin{align}
\label{eq:one_state_occ_eq}
(1 - \hat{P}^{(n)}_{\rm h/p})\hat{f}^{(n)} (1 - \hat{P}^{(n)}_{\rm h/p})|\phi_i^{(n)}\rangle &= \epsilon^{(n)}_i |\phi_i^{(n)}\rangle,\\
\label{eq:one_state_hole_eq}
\hat{P}^{(0)}(1-\hat{Q}_s^{(n)}) \hat{f}^{(n)} (1-\hat{Q}_s^{(n)})\hat{P}^{(0)} |\phi_h^{(n)}\rangle &= \epsilon^{(n)}_{\rm h} |\phi_{\rm h}^{(n)}\rangle,\\
\label{eq:one_state_part_eq}
\hat{Q}^{(0)}(1-\hat{P}_s^{(n)}) \hat{f}^{(n)} (1-\hat{P}_s^{(n)}) \hat{Q}^{(0)}|\phi_{\rm p}^{(n)}\rangle &= \epsilon^{(n)}_{\rm p} |\phi_{\rm p}^{(n)}\rangle.
\end{align}
