...
The cost to compute $d$ excited states in OCDFT is proportional to $d N^3$ and $d N^4$ for pure and hybrid functional, respectively, since for each excited states one has to solve a set of constrained Kohn--Sham equations.
...
%\begin{mdframed}[hidealllines=true,backgroundcolor=yellow!70]
...
Previous computational studies of CO$_2$\cite{Maganas:ji} showed that B3LYP/TD-DFT underestimates the oxygen K-edge by 14 eV.
...
\hilight{Two classes of systems that are worth further exploration are transition metal complexes and open-shell molecules. Core excitations in transition metal complexes present a number of additional challenges that will require further extension of the theory.
...
The simulation of L-edge spectra presents additional challenges\cite{roemelt_combined_2013} due to the strong mixing of excitation from degenerate 2p core orbitals and the necessity to account for the coupling of molecular multiplets that experience strong spin-orbit coupling.
...
These considerations also apply to species with a high-spin open-shell ground state and whose excited states that cannot be represented by a single Slater determinant.
A case that is more problematic is that of molecules with a low-spin open-shell ground state.
In this situation, if the ground state DFT calculation yields an unphysical result, than it will be unlikely for OCDFT to yield accurate excitation energies.}
